  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 finalColor = color * (0.45 + diffuse * 0.9) + color * fresnel * 0.85;
    ...
    const outline = new THREE.LineSegments(
      ...
      new THREE.LineBasicMaterial({
        color: new THREE.Color("hsl(203 92% 68%)"),
        transparent: true,
        opacity: 0.72,
      })
    );
    ...
    const ambientLight = new THREE.AmbientLight(0x8bb8ff, 0.55);
...
export function AnomalousMatterHero({
  ...
  return (
    <section
      role="banner"
      className="relative h-screen w-full overflow-hidden bg-[var(--background)] text-[var(--foreground)]"
    >
      <Suspense
        fallback={<div className="h-full w-full bg-[var(--background)]" />}
      >
        <GenerativeArtScene />
      </Suspense>

      <div className="absolute inset-0 z-10 bg-gradient-to-t from-[var(--background)] via-[color-mix(in_oklab,var(--background)_70%,transparent)] to-transparent" />
      ...
      <div className="relative z-20 flex h-full flex-col items-center justify-end pb-20 text-center md:pb-32">
        <div className="animate-fade-in-long max-w-3xl px-4">
          <h1 className="font-mono text-sm uppercase tracking-widest text-[hsl(var(--sky-300)/0.8)]">
            {title}
          </h1>
          <p className="mt-4 text-3xl font-bold leading-tight md:text-5xl">
            {subtitle}
          </p>
          <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-[hsl(var(--gray-300)/0.8)]">
            {description}
          </p>
        </div>
      </div>
